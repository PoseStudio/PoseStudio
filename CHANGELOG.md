...
### Changed
* **Content-library resolution for figures.** The figure importer resolves a preset's cross-file references against the content-library root(s): the folder auto-detected from the imported file's own location, **unioned** with any roots the user has configured. When a figure is browsed from a presets-only folder (with no co-located `data/` library), the interactive import now offers a **"Locate Content Library…"** prompt, remembers the chosen root, and retries — so those figures import instead of failing silently. A file that isn't a figure (pose / material / light presets share the extension) reports a clear message rather than doing nothing.
